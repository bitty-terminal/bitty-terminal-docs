...
> identity / focus, following research recommendation 3 in
> `recording/research/017.md.completed` (M4 invariant hardening). It is
> checked read-only against `bitty` `origin/main` revision
> `706fa25` (2026-09-14), the base of the linked `ctx-0405` pull request that
> carries the executable checks. Rules are marked **Accepted** when an
...
| Tested                | An executable check in `bitty` covers the invariant at revision `706fa25` plus the linked `ctx-0405` pull request.          |
...
Two layers exist in `bitty` at revision `706fa25`, and their invariants are
distinct:
...
| WS-INV-6 | A workspace slot keeps a stable creation sequence (`WorkspaceSlot::seq`) across index shifts, and the workspace MRU holds each live index exactly once.                             | Implementation-only | Partially tested — `workspaces.rs` inline `new_switch_prev_next_last_update_tabline`, `switch_preserves_per_workspace_layouts`; `seq` stability has no dedicated check (follow-up F-4)                      |

### Ownership

| ID        | Invariant                                                                                                                                                                                                                      | Status              | Coverage                                                                                                                                                                                                                                                                                                                                                                       |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| WS-INV-7  | Every grid has exactly one owner: a pane session is keyed by exactly one live `ViewId`; the primary grid is owned by at most one live leaf (`primary_view`), and no workspace close may leave the owner pointing at a dead id. | Accepted, refined   | Tested — `panel_session_invariants.rs::randomized_live_ops_preserve_session_ownership_and_geometry`, `primary_owner_rehomes_only_on_close`                                                                                                                                                                                                                                     |
...
| WS-INV-16 | A workspace switch preserves each slot's stashed layout and focus; the active mirror equals the loaded slot; switching is focus-valid on every slot.                                                    | Accepted            | Tested — `workspaces.rs` inline `switch_preserves_per_workspace_layouts`, `new_switch_prev_next_last_update_tabline`; registry `inactive_workspace_visibility_not_rendered_but_retains_attachment`; randomized sequences scan every slot                                                                                             |
...
| WS-INV-19 | Removing or destroying the focused leaf re-homes focus before the edit commits; an explicit close of the primary owner re-homes the primary grid to the focused survivor; focus moves never re-home the owner.    | Accepted, refined | Tested — registry `focus_mru_survives_detach_and_destroy`, `create_view_focuses_the_new_view`; `panel_session_invariants.rs::primary_owner_rehomes_only_on_close`; randomized live sequence (workspace close)                          |
...
| WS-INV-24 | Resize routing is view-rectangle driven, debounced, and revalidated: at most one committed resize per presentation tick, coalescing bounded at 64 queued rectangles, and every committed resize bumps damage generation.                                                                                                                      | Accepted  | Tested — registry `debounce_64_coalesces_and_counts`, `zero_area_never_reaches_pty_and_retains_previous_geometry`; `runtime_resize.rs` suite                                                                                                                                                                              |

### Reload and persistence boundaries

| ID        | Invariant                                                                                                                                                                                                                  | Status              | Coverage                                                                                                                                                                                             |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WS-INV-25 | Reload never resurrects a live PTY or `RuntimeId`. Session-scoped facts (scrollback capacity, shell recipe, pane sessions) take effect at the next session creation; a process restart always allocates fresh ids.         | Accepted, candidate | Partially tested — `panes.rs` captures scrollback capacity at session creation; `background_images.rs` whole-config reload gate. No runtime-level session/layout rehydration exists (follow-up F-2). |
| WS-INV-26 | Rehydration creates a fresh `TerminalId` and `RuntimeId` under the same `PersistentId`; a second live terminal with the same `PersistentId` is rejected; rehydrated scrollback is immutable history, never live PTY state. | Accepted            | Tested at registry scope — `reattachment_vs_recreation`, `persistent_id_validation_and_in_use`, `stale_handle_rejected_with_expected_and_found`; not wired into the live runtime (F-2).              |
| WS-INV-27 | `bitty --safe` opens no background image and keeps an ephemeral, bounded registry; no persisted resource is loaded implicitly.                                                                                             | Accepted            | Tested — `background_images.rs` safe-startup gate; `workspace_panel.rs::safe_mode_rejects_workspace_without_panic`                                                                                   |

## Verification map

| Check                                                                                                                                                                         | Covers                                               |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `cargo test -p bitty-runtime --test panel_invariants --locked`                                                                                                                | WS-INV-1, 12, 13, 14, 15, 18 (headless randomized)   |
...
| `cargo test -p bitty-runtime --test pane_sessions --test pane_damage --test runtime_layout --test runtime_resize --test split_live_reflow --test split_zoom_repaint --locked` | WS-INV-8, 9, 10, 17, 20, 21, 22, 23, 24              |
...
| F-2 | WS-INV-25/26 runtime persistence: no workspace-snapshot serialization of layout, attachment map, focus MRU, or scrollback rehydration exists in `bitty-runtime`; only the experimental registry implements the IDs involved.                                                                                | Follow-up task after the persistence design is scoped; keep the accepted RFC's boundaries.                                                      |
...
- Research note 017 (`recording/research/017.md.completed`) — M4 invariant-hardening recommendation that produced this document.
