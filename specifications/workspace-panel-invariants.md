...
| WS-INV-16 | A workspace switch preserves each slot's stashed layout and focus; the active mirror equals the loaded slot; switching is focus-valid on every slot.                                                    | Implementation-only | Tested — `workspaces.rs` inline `switch_preserves_per_workspace_layouts`, `new_switch_prev_next_last_update_tabline`; registry `inactive_workspace_visibility_not_rendered_but_retains_attachment`; randomized sequences scan every slot                                                                                             |
