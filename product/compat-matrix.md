...
- Companion: [`compat-lab.md`](./compat-lab.md) (Phase C scaffold, CTX-0074), [`manual-smoke.md`](./manual-smoke.md) (human checklist, CTX-0087/0099), [`dogfooding.md`](./dogfooding.md) (daily-driver headless, CTX-0077), `crates/bitty-compat-lab` (owner, `forbid(unsafe)`, bounded 8 KiB/4096), the machine-readable matrix JSON (generated headlessly into the git-ignored workspace evidence area), per-category snapshot dumps (39+ dumps via `collect_dumps` into the evidence area).
- Maintained M1/M2 matrix: [`reference/compatibility-matrix.md`](../reference/compatibility-matrix.md) (CTX-0003/CTX-0404) is the per-scenario evidence register with `ci`/`local`/`partial`/`gap` status; this document remains the release-instance snapshot of the 14-surface differential.
...
All rows `self PASS` via `cargo test -p bitty-compat-lab --test compat_matrix` (bounded, headless, `forbid(unsafe)`) and `cargo test -p bitty-compat-lab --test compare -- --nocapture` `total 39 self_passed 39`. Reference columns `SKIP` when no evidence-area per-backend snapshot dump exists per `corpus_rel` (CI-friendly); when present, `compare_all` asserts text equality and counts `ref_passed`/`ref_failed`. Pixel diff is explicitly out of scope (snapshot text diff only).
...
- Reference clone provenance summarized inline (umbrella evidence-area revision table) — `ghostty 8867c37` MIT, `kitty 087b8c3` GPL-3.0, `wezterm f93d903` MIT, `alacritty ede2ac1` Apache-2.0/MIT, `xterm 9489b20` MIT/X11, `vttest 3.4.0` synthetic. No clone is executed or imported.
- Comparator backends probed in order `ghostty` → `kitty` → `wezterm` → `alacritty` (new in CTX-0114). Candidates are per-backend evidence-area directories (`workspace_root` evidence area plus umbrella mirrors); discovery is sorted, bounded to `MAX_SNAPSHOTS 64` and `MAX_SNAPSHOT_JSON_BYTES 16 KiB`.

## Recordings corpus and regression tests

- **Corpus**: `tests/compat/*/corpus/*.bin` (30 baseline + 9 dogfooding = 39 as of `1d9eb6a`, plus placeholders). Each ≤8 KiB, each re-parse byte-by-byte deterministic, each `state_hash` canonical. New in CTX-0114: no new `.bin` required — existing 39 already cover all 14 surfaces via dogfooding corpus. `cargo run -p bitty-compat-lab --bin collect_dumps --locked` regenerates the 39 per-category snapshot dumps (`<16 KiB` each, `80x24`, `CANONICAL_HASH_VERSION 1`) in the worktree evidence area plus the umbrella mirror (`$BITTY_WORKSPACE`, else the repo parent).
- **Machine-readable matrix**: dated matrix JSON (generated headlessly into the git-ignored workspace evidence area via `crates/bitty-compat-lab/src/matrix.rs` `generate_matrix_json()` or `cargo test -p bitty-compat-lab --test compat_matrix -- --nocapture` artifact). Contains per-surface `corpus_rel`, `bytes_len`, `actions_len`, `state_hash`, `snapshot_width/height`, `generation`, `self PASS`, `reference SKIPPED/PASS` per backend. Bounded `<16 KiB`.
...
- Run header: `date: 2026-09-01  host: cachyos-hyprland  bitty: 1d9eb6a+ (carryctx/ctx-0114)` reference revisions summarized inline (ghostty `8867c37`, kitty `087b8c3`, wezterm `f93d903`, alacritty `ede2ac1`).
...
- Snapshots: `cargo run -p bitty-compat-lab --bin collect_dumps --locked` wrote 39 snapshots to the ephemeral dump directory mirrored to the worktree and umbrella evidence areas, each `<16 KiB`, `80×24`, `CANONICAL_HASH_VERSION 1`, deterministic.
...
- Matrix artifact: dated matrix JSON (14 entries, `<16 KiB`, sorted keys, bounded; generated into the git-ignored workspace evidence area, tracked v1 copies removed in CTX-0379) + this doc table.
