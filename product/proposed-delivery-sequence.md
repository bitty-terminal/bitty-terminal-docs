...
- Status: draft research record. This document remains a **proposal** retained
  for evaluation: its sequence, lists, criteria, and staging language are not
  accepted direction, a roadmap commitment, or authorization to implement.
  Where this record names accepted staging, that status belongs only to the
  separately accepted artifact, [ADR 0008](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/adrs/ADR-0008-headless.md),
  and is not conferred by this draft.
...
- Roadmap relation: the [Roadmap index](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/roadmap/README.md) admits roadmap
...
### Spine implementation presence (as of 2026-09-21, crates present does not imply acceptance)

The `bitty` workspace is now spine-complete in crate presence (twenty-one members
in `bitty/Cargo.toml`: sixteen after the `bitty-lua` addition at CTX-0040
`d67a65b`, plus `bitty-compat-lab`, `bitty-perf`, `bitty-panels`,
`bitty-test-support`, and `bitty-test-vm`; `bitty-lua` now runs the `phodopus`
VM after the piccolo-to-Phodopus swap #1204). Presence tracks the candidate spine above; most stages remain
...
CTX-0040 (21 + 15 headless tests, `just check` 92 files 0 issues, `cargo check --target x86_64-pc-windows-gnu` pass) and DevTools instrumentation/event pipeline/debug protocol accepted via CTX-0053 (`just check` 91 files 0 issues). Pivot-wave
additions (all `Implemented`, not `Verified`): RC-11 store quota backend
(#1200), lifecycle ladder with enforcement records and reload ordering
(#1201), fail-closed load gate with stable budget codes (#1202), and the
piccolo-to-Phodopus swap with de-piccolo of `event.rs` plus readiness mirror
suite (#1204/#1206/#1207). No entry below self-accepts its
contract:
...
| Config                         | `bitty-config`                                        | Present; typed `ConfigPlan`/validation/migration/reload/project-trust accepted in [Configuration Model RFC](../specifications/configuration-model-rfc.md) (OQ-010, 2026-08-27); Lua runtime, sandbox, and diagnostics contracts accepted in [Lua Runtime RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/runtime/lua-runtime-rfc.md) (OQ-009, 2026-08-27; OQ-030, OQ-031, OQ-032 remain Open as follow-ups)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Command/Event + Plugin Runtime | `bitty-plugin-host` + `bitty-lua`                     | Present; registry/capability/lifecycle and event pipeline accepted in [Plugin Platform RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/specifications/plugin-platform-rfc.md) (OQ-011/OQ-012/OQ-013, 2026-08-27); `bitty-package` edge added; `mlua`/`piccolo` choice resolved to `piccolo` 0.3.3 in `bitty-lua` (CTX-0040 `d67a65b`, worktree `ctx-0040/feat-lua-vm-budgets`, 15->16 members, `forbid(unsafe_code)`) OQ-014 enforcement tuning is `Accepted` 2026-08-28; DropOldest is the accepted v1 default, three-level budgets aligned with `BoundedText` strict — measured 2026-08-27 via `crates/bitty-plugin-host/tests/measurement.rs` (21 headless tests, `global_events_enforced_*`, instrumentation `budget_snapshot`/`invariant_*`/`invariant_global_bounds`) plus `crates/bitty-lua/tests/measurement_lua.rs` (15 headless tests, `piccolo` Fuel/wall, `VmBudgetSnapshot`/`would_exceed_lua_limits`) proving per-subscription 64 strict, per-plugin 1024/256 KiB DropOldest, Global 8192/2 MiB hard-gated enforced at Host admission (`EventPipeline::publish` / `Host::publish` via `would_exceed_global_limits` + `evict_oldest_globally`, strict `invariant_global_bounds`), RC-1 10^7/50ms/8ms warning hard-gated via `piccolo` Fuel + wall deadline, RC-2 32 MiB via `Lua::total_memory()`, `drain_batch` strict; queue budgets and VM budgets now enforced measured and accepted (frontmatter `accepted`, closed OQ-014 2026-08-28, gates `just check` 92 files 0 issues + `cargo check --target x86_64-pc-windows-gnu` pass @ `d67a65b`); pivot-wave additions (all `Implemented`, not `Verified`): RC-11 store quota backend (`store.rs`, #1200), fail-closed load gate with stable budget codes (`gate.rs`/`error.rs`, #1202), piccolo-to-Phodopus VM swap with de-piccolo of `event.rs` and readiness mirror suite (#1204/#1206/#1207), and host-owned lifecycle ladder with enforcement records and reload ordering (`lifecycle.rs`, #1201) |
| Plugin Manager                 | `bitty-plugin-host`, `bitty-package`                  | `bitty-package` lifecycle and integrity model accepted in [Package Lifecycle RFC](https://github.com/bitty-terminal/bitty-plugins-docs/blob/main/packaging/package-lifecycle-rfc.md) (OQ-021, 2026-08-27); real signature verification and registry/key items remain draft under OQ-022 and OQ-026 through OQ-029                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| IPC                            | `bitty-ipc`                                           | Draft std-only stub; bounded framing/channels/stdio. Wire/auth/scopes are accepted at the design level in the [IPC and Agent RFC](https://github.com/bitty-terminal/bitty-ai-docs/blob/main/specifications/ipc-agent-rfc.md), which closes [OQ-018](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md). Implementation is recorded as `Implemented` at `be3bdb4`, but not yet `Verified`, release-ready, or user-ready.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Agent                          | `bitty-agent`                                         | Draft std-only stub; bounded messages/side queue. Auth/consent/streaming are accepted at the design level in the [IPC and Agent RFC](https://github.com/bitty-terminal/bitty-ai-docs/blob/main/specifications/ipc-agent-rfc.md), which closes [OQ-018](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md). Implementation is recorded as `Implemented` at `be3bdb4`, but not yet `Verified`, release-ready, or user-ready; OQ-019 is `Accepted` via [DevTools RFC](../specifications/devtools-rfc.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
[open-question register](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md) with category,
...
- Platforms: Tier 1 Linux, Windows, and macOS plus Tier 2 BSD, consistent with
  the platform tiers accepted in [ADR 0002](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/adrs/ADR-0002-platform-support-tiers.md).
  The M1 compatibility matrix runs on all Tier 1 legs (#1172) as `Implemented`
  evidence, not a `Verified` claim.
- Shell coverage: compatibility across six shells (bash, zsh, fish, PowerShell,
  cmd, nushell); which shells belong to the
  first milestone remains governed by
  [OQ-004](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md) and the
  [Compatibility Milestone RFC](../specifications/compatibility-milestone-rfc.md).
  Six-shell PTY evidence exists (`m1_shell_coverage.rs`, a `scripts/m1-matrix.sh`
  member, #1188) as `Implemented` evidence, not a `Verified` claim.
...
- Security gates already normative in the
  ...
  [threat model](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/security/threat-model.md), such as parser limits,
...
For the headless `bittyd` daemon question, this draft records the staging that is
accepted only by [ADR 0008](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/adrs/ADR-0008-headless.md). This
document does not accept that staging. ADR 0008 defers the daemon to post-v1.0,
accommodates the headless runtime early, and gates any future daemon or remote
work on a mandatory trust-boundary analysis:

- Positioning: after v1.0, or at earliest near v1.0; accommodate early, do not implement early (accepted only via ADR 0008).
...
This staging was offered to [OQ-020](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md) as a
candidate and is deferred to post-v1.0 with a trust-boundary gate only because
[ADR 0008](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/adrs/ADR-0008-headless.md) accepts that disposition. The
trust-boundary half requires analysis against the [threat model](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/security/threat-model.md)
per that ADR. The existing candidate bullet in the [Architecture Overview](../architecture/overview.md)
long-term evolution section cites ADR 0008 as the accepted daemon positioning;
this draft remains neither accepted nor a release or user-ready claim.

## Relationship to registers

- No open question is closed or answered by this record, and no entry in the
  [decision register](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/index.md) changes status.
- Acceptance of any item requires the reviewable artifact named in the
  [open-question register](https://github.com/bitty-terminal/bitty-docs/blob/main/docs/decisions/open-questions.md) or a new scoped
  ADR/RFC.
...
- Sync note (2026-09-21, `bitty` `491c567`, post pivot-wave #1200-1213): the
  workspace now has 21 members; `bitty-ipc` is `publish = true` (ten
  `publish = true` / eleven `publish = false`); `bitty-panels`,
  `bitty-test-support`, and `bitty-test-vm` joined as `publish = false`;
  `bitty-lua` runs the `phodopus` VM (piccolo-to-Phodopus swap #1204). The
  CTX-0044 group table above remains the proposal record, not current publish
  state.
