...
`bitty-terminal.*` as non-builtin without panic, identical to `example.*` parity.
...
- **Built-in namespace prefix for Panel plugins**: `PluginHost::declare` safe-mode
  ...
  in safe mode (parity with third-party `example.*`). Future file-manager/git/browser
